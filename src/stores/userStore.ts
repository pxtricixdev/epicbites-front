...
export const useUserStore = defineStore('users', () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await fetch('https://epicbitesapi.retocsv.es/api/users', {
        method: 'GET',
        headers: {
  ...
  const updateUser = async (userData: Partial<IPutUser>, userId?: number) => {
    ...
    try {
      ...
      const response = await fetch(`https://epicbitesapi.retocsv.es/api/users/${targetUserId}`, {
        method: 'PATCH',
        headers: {
          accept: 'text/plain',
  ...
  const deleteUser = async (id: number) => {
    ...
    try {
      const response = await fetch(`https://epicbitesapi.retocsv.es/api/users/${id}`, {
        method: 'DELETE',
        headers: {
          accept: 'application/json',
          'Content-Type': 'application/json',

...
export const useRecipeNoteStore = defineStore('recipeNotes', () => {
  ...
  const fetchAllNotes = async () => {
    ...
    error.value = null
    try {
      const response = await fetch('https://localhost:7129/api/recipe-notes', {
        method: 'GET',
        headers: {
  ...
  const fetchNotesByUser = async (userId: number) => {
    ...
    error.value = null
    try {
      const response = await fetch(`https://localhost:7129/api/recipe-notes/user/${userId}`, {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
  ...
  const deleteNote = async (id: number) => {
    ...
    error.value = null
    try {
      const response = await fetch(`https://localhost:7129/api/recipe-notes/${id}`, {
        method: 'DELETE',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
